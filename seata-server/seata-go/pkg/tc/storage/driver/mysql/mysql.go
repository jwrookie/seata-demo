...
import (
	...
	_ "github.com/go-sql-driver/mysql" // register scripts
...
func init() {
	factory.Register("scripts", &mysqlFactory{})
}
...
func New(params DriverParameters) (storage.Driver, error) {
	...
	engine, err := xorm.NewEngine("scripts", params.DSN)
	if err != nil {
		return nil, err
	}
...
func (driver *driver) AcquireLock(rowLocks []*apis.RowLock, skipCheckLock bool) bool {
	...
	if err != nil {
		...
		// but the scripts driver reports invalid connection exception, and then re-registers the branch,
